...
class Graph extends Component {
    ...
    findPath = (graph, start, end) => {
      //initialize arrays for info sorting/storage and the found value to 
        ...
        path.push(checked[checked.length - 1])
        do{
          path.push(checked.find((ele) => {
            return ele.id === path[path.length - 1].breadCrumb
          }))
        } while(path[path.length - 1].breadCrumb);
        console.log(path)
        return path
      }

    render(){
        const {radius, grid, graph, hexPathStart, hexPathEnd} = this.state
        return(
                                ...
                                <div key={i} className='hex-column'>
                                    {ele.map((hex) => {
                                        return(
                                            ...
                                            key={hex.id} 
                                            className='hex-1'>x:{hex.x} y:{hex.y} z:{hex.z} </div>
                                        )
                                    })}
                                </div>
                            )
                                ...
                                <div key={i} className='hex-column'>
                                    {ele.map((hex) => {
                                        return(
                                            <div onClick={() => this.setHexPath(hex)} 
                                            key={hex.id} 
                                            className='hex-2'>x:{hex.x} y:{hex.y} z:{hex.z}</div>
                                        )
                                    })}
                                </div>
                            )
